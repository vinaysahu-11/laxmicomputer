...
const QuestionDownloadCard = ({ paper }) => {
  return (
    <div className="bg-surface-container-lowest border border-outline-variant/60 rounded-xl p-4 shadow-sm text-left flex justify-between items-center gap-4 transition-all duration-200 hover:shadow-md">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <h4 className="font-label-sm text-label-sm font-bold text-on-surface leading-tight truncate">{paper.title}</h4>
          <span className="text-[9px] text-on-surface-variant font-bold uppercase tracking-wider block mt-1.5">{paper.year} Solver • {paper.size}</span>
        </div>
      </div>
...
export default QuestionDownloadCard;
